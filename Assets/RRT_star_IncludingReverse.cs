...
public class RRT_star_IncludingReverse : MonoBehaviour
{

    public float stepSize;
    public float xLow, xHigh, zLow, zHigh;
    public float yLevel, lineWidth;
    public float maxIterations = 10000;
    [HideInInspector] public float timestep = 0.0001f;
    private Tree theTree;
    [HideInInspector] public List<Node> pathWayPoints = new List<Node>();
    public static List<Vector3> AllPathWaypoints = new List<Vector3>();
    public static List<bool> AllPathWaypointsIsReverse = new List<bool>();
    [HideInInspector] public List<Vector3> ReferencePath = new List<Vector3>();
    [HideInInspector] public List<bool> ReferencePathIsReverse = new List<bool>();
    ...
    [HideInInspector] public bool step = false, disableStepping = false;
    private bool startkey = false;
    private int dimension = 2; // 3차원 드론을 날리면 3으로 바꾸자.(그럴일 없겠지만)
    public float gammaRRT = 30f; //최소값은 2*(1+1/dimension)^(1/dimension) *(area of free space / pi)^(1/dimension) = 138.197
    ...
    public float BackPenalty = 10;
    public static float rev_penalty = 10;
                ...
                if (IsColliding(nearest.position, randPoint) != -1)
                {
                    ...
                    if (newpoint.goal && newpoint.cost < float.MaxValue / 100000)
                    {
                        if (minGoalCost > newpoint.cost)
                        {
                            ...
                            ReferencePathIsReverse = AllPathWaypointsIsReverse;
    ...
    //Need improvement 이게 진짜 메인 파트다..... 잘 조절 하면 효율 쭉쭉 증가
    public static float CalculateDirection(Node from, ref Node to)
    {
        //return Random.Range(-Mathf.PI,Mathf.PI);
        //return Mathf.Atan2(to.position.z - from.position.z, to.position.x - from.position.x) + Random.Range(-Mathf.PI/6,Mathf.PI/6);
		//if(Random.Range(0,1)<0.5){

		//return ((from.facedirection+Mathf.PI) + 2f * (Mathf.Atan2(to.position.z - from.position.z, to.position.x - from.position.x) - (from.facedirection+Mathf.PI))) % (2 * Mathf.PI) + Random.Range(-Mathf.PI/6,Mathf.PI/6) + Mathf.PI;
		return ((from.facedirection+Mathf.PI) + 2f * (Mathf.Atan2(to.position.z - from.position.z, to.position.x - from.position.x) - (from.facedirection+Mathf.PI))) % (2 * Mathf.PI) + GaussianRandom.generateNormalRandom(0,Mathf.PI/12) + Mathf.PI;
        
        //}
		//else{
		//	return (from.facedirection + 1.5f * (Mathf.Atan2(to.position.z - from.position.z, to.position.x - from.position.x) - from.facedirection)) % (2 * Mathf.PI) + Random.Range(-Mathf.PI/6,Mathf.PI/6) + Mathf.PI;
		//}
    ...
    public static class GaussianRandom
{
    ...
    public class Tree
    {
        ...
        public void DrawCompletedPath(ref List<Node> pathWayPoints, float turnRadius)
        {
            ...
            AllPathWaypointsIsReverse.Clear();
    ...
    {
        public enum PathType { RSR, LSL, RSL, LSR, RLR, LRL, RSR_Rev, LSL_Rev, RSL_Rev, LSR_Rev, RLR_Rev, LRL_Rev };
        ...
        private static SingleDubinsPath GetRSR_Rev()
        {
            Vector3 startTangent = Vector3.zero;
            Vector3 goalTangent = Vector3.zero;
            float turnRadius = startCar.turnRadius;

            Dubinsmath.RSRorLSL(goalRightCircle, startRightCircle, out startTangent, out goalTangent, turnRadius, false);

            float length1 = Dubinsmath.GetArcLength(goalRightCircle, goalCar.pos, startTangent, false, turnRadius);
            if (Circle.checkCircle(goalRightCircle, goalCar.pos, startTangent, false, turnRadius))
            {
                return null;
            }
            float length2 = (startTangent - goalTangent).magnitude;
            ...
            float length3 = Dubinsmath.GetArcLength(startRightCircle, goalTangent, startCar.pos, false, turnRadius);
            if (Circle.checkCircle(startRightCircle, goalTangent, startCar.pos, false, turnRadius))
            {
                return null;
            }

            SingleDubinsPath pathData = new SingleDubinsPath((length1 + length2 + length3) * rev_penalty, goalRightCircle, goalCar.pos,
                                                            startRightCircle, startCar.pos, startTangent, goalTangent, PathType.RSR_Rev);

            return pathData;
        }

        private static SingleDubinsPath GetLSL_Rev()
        {
            Vector3 startTangent = Vector3.zero;
            Vector3 goalTangent = Vector3.zero;
            float turnRadius = startCar.turnRadius;

            Dubinsmath.RSRorLSL(goalLeftCircle, startLeftCircle, out startTangent, out goalTangent, turnRadius, true);

            float length1 = Dubinsmath.GetArcLength(goalLeftCircle, goalCar.pos, startTangent, true, turnRadius);
            if (Circle.checkCircle(goalLeftCircle, goalCar.pos, startTangent, true, turnRadius))
            {
                return null;
            }
            float length2 = (startTangent - goalTangent).magnitude;
            ...
            float length3 = Dubinsmath.GetArcLength(startLeftCircle, goalTangent, startCar.pos, true, turnRadius);
            if (Circle.checkCircle(startLeftCircle, goalTangent, startCar.pos, true, turnRadius))
            {
                return null;
            }

            SingleDubinsPath pathData = new SingleDubinsPath((length1 + length2 + length3) * rev_penalty, goalLeftCircle, goalCar.pos,
                                                            startLeftCircle, startCar.pos, startTangent, goalTangent, PathType.LSL_Rev);

            return pathData;
        }

        private static SingleDubinsPath GetRSL_Rev()
        {
            Vector3 startTangent = Vector3.zero;
            Vector3 goalTangent = Vector3.zero;
            float turnRadius = startCar.turnRadius;

            Dubinsmath.RSLorLSR(goalRightCircle, startLeftCircle, out startTangent, out goalTangent, turnRadius, false);
            ...
            SingleDubinsPath pathData = new SingleDubinsPath((length1 + length2 + length3) * rev_penalty, goalRightCircle, goalCar.pos,
                                                             startLeftCircle, startCar.pos, startTangent, goalTangent, PathType.RSL_Rev);

            return pathData;
        }
        private static SingleDubinsPath GetLSR_Rev()
        {
            Vector3 startTangent = Vector3.zero;
            Vector3 goalTangent = Vector3.zero;
            float turnRadius = startCar.turnRadius;

            Dubinsmath.RSLorLSR(goalLeftCircle, startRightCircle, out startTangent, out goalTangent, turnRadius, true);
            ...
            SingleDubinsPath pathData = new SingleDubinsPath((length1 + length2 + length3) * rev_penalty, goalLeftCircle, goalCar.pos,
                                                             startRightCircle, startCar.pos, startTangent, goalTangent, PathType.LSR_Rev);

            return pathData;
        }

        private static SingleDubinsPath GetRLR_Rev()
        {
            Vector3 startTangent = Vector3.zero;
            Vector3 goalTangent = Vector3.zero;
            Vector3 middleCircle = Vector3.zero;
            ...
            Dubinsmath.RLRorLRL(goalRightCircle, startRightCircle, out startTangent, out goalTangent, out middleCircle, turnRadius, true);
            ...
            SingleDubinsPath pathData = new SingleDubinsPath((length1 + length2 + length3) * rev_penalty, goalRightCircle, goalCar.pos,
                                                            startRightCircle, startCar.pos, startTangent, goalTangent, PathType.RLR_Rev);

            return pathData;
        }
        private static SingleDubinsPath GetLRL_Rev()
        {
            Vector3 startTangent = Vector3.zero;
            Vector3 goalTangent = Vector3.zero;
            Vector3 middleCircle = Vector3.zero;
            ...
            Dubinsmath.RLRorLRL(goalLeftCircle, startLeftCircle, out startTangent, out goalTangent, out middleCircle, turnRadius, false);
            ...
            if (Circle.checkCircle(goalLeftCircle, goalCar.pos, startTangent, true, turnRadius)
             || Circle.checkCircle(middleCircle, startTangent, goalTangent, false, turnRadius)
            || Circle.checkCircle(startLeftCircle, goalTangent, startCar.pos, true, turnRadius))
            {
                return null;
            }

            SingleDubinsPath pathData = new SingleDubinsPath((length1 + length2 + length3) * rev_penalty, goalLeftCircle, goalCar.pos,
                                                            startLeftCircle, startCar.pos, startTangent, goalTangent, PathType.LRL_Rev);
    ...
    public class SingleDubinsPath
    {
        ...
        public void DrawSingleDubinsPath(float turnRadius, float yL)
        {
            ...
            {
                Circle.DrawCircle(goalCircle, turnRadius, tanGoal, goalPos, false, Color.blue, lWidth, yL, false);
                Line.DrawLine(tanStart, tanGoal, Color.blue, lWidth, yL, false);
                Circle.DrawCircle(startCircle, turnRadius, startPos, tanStart, false, Color.blue, lWidth, yL, false);
            }
            if (this.pathType == Dubinspath.PathType.LSL)
            {
                Circle.DrawCircle(goalCircle, turnRadius, tanGoal, goalPos, true, Color.blue, lWidth, yL, false);
                Line.DrawLine(tanStart, tanGoal, Color.blue, lWidth, yL, false);
                Circle.DrawCircle(startCircle, turnRadius, startPos, tanStart, true, Color.blue, lWidth, yL, false);
            }
            ...
            if (this.pathType == Dubinspath.PathType.RLR)
            {
                ...
                Circle.DrawCircle(middleCircle, turnRadius, tanStart, tanGoal, true, Color.blue, lWidth, yL, false);
                Circle.DrawCircle(startCircle, turnRadius, startPos, tanStart, false, Color.blue, lWidth, yL, false);
            }
            if (this.pathType == Dubinspath.PathType.LRL)
            {
                Circle.DrawCircle(goalCircle, turnRadius, tanGoal, goalPos, true, Color.blue, lWidth, yL, false);
                Vector3 middleCircle = 2 * tanStart - startCircle;
                Circle.DrawCircle(middleCircle, turnRadius, tanStart, tanGoal, false, Color.blue, lWidth, yL, false);
                Circle.DrawCircle(startCircle, turnRadius, startPos, tanStart, true, Color.blue, lWidth, yL, false);
            }

            if (this.pathType == Dubinspath.PathType.RSR_Rev)
            {
                Circle.DrawCircle(startCircle, turnRadius, tanStart, startPos, true, Color.red, lWidth, yL, true);
                Line.DrawLine(tanGoal, tanStart, Color.red, lWidth, yL, true);
                Circle.DrawCircle(goalCircle, turnRadius, goalPos, tanGoal, true, Color.red, lWidth, yL, true);
            }
            if (this.pathType == Dubinspath.PathType.LSL_Rev)
            {
                Circle.DrawCircle(startCircle, turnRadius, tanStart, startPos, false, Color.red, lWidth, yL, true);
                Line.DrawLine(tanGoal, tanStart, Color.red, lWidth, yL, true);
                Circle.DrawCircle(goalCircle, turnRadius, goalPos, tanGoal, false, Color.red, lWidth, yL, true);
            }
            if (this.pathType == Dubinspath.PathType.RSL_Rev)
            {
                Circle.DrawCircle(startCircle, turnRadius, tanStart, startPos, true, Color.red, lWidth, yL, true);
                Line.DrawLine(tanGoal, tanStart, Color.red, lWidth, yL, true);
                Circle.DrawCircle(goalCircle, turnRadius, goalPos, tanGoal, false, Color.red, lWidth, yL, true);
            }
            if (this.pathType == Dubinspath.PathType.LSR_Rev)
            {
                Circle.DrawCircle(startCircle, turnRadius, tanStart, startPos, false, Color.red, lWidth, yL, true);
                Line.DrawLine(tanGoal, tanStart, Color.red, lWidth, yL, true);
                Circle.DrawCircle(goalCircle, turnRadius, goalPos, tanGoal, true, Color.red, lWidth, yL, true);
            }
            if (this.pathType == Dubinspath.PathType.RLR_Rev)
            {
                Circle.DrawCircle(startCircle, turnRadius, tanStart, startPos, true, Color.red, lWidth, yL, true);
                Vector3 middleCircle = 2 * tanStart - startCircle;
                Circle.DrawCircle(middleCircle, turnRadius, tanGoal, tanStart, false, Color.red, lWidth, yL, true);
                Circle.DrawCircle(goalCircle, turnRadius, goalPos, tanGoal, true, Color.red, lWidth, yL, true);
            }
            if (this.pathType == Dubinspath.PathType.LRL_Rev)
            {
                Circle.DrawCircle(startCircle, turnRadius, tanStart, startPos, false, Color.red, lWidth, yL, true);
                Vector3 middleCircle = 2 * tanStart - startCircle;
                Circle.DrawCircle(middleCircle, turnRadius, tanGoal, tanStart, true, Color.red, lWidth, yL, true);
                Circle.DrawCircle(goalCircle, turnRadius, goalPos, tanGoal, false, Color.red, lWidth, yL, true);
            }
    ...
    {
        public static void DrawLine(Vector3 start, Vector3 end, Color color, float lWidth, float yL, bool isReverse)
        {
            GameObject myline = new GameObject();
            myline.AddComponent<LineRenderer>();
            ...
            while (i * howDenseLine < (start - end).magnitude)
            {
                ...
                AllPathWaypointsIsReverse.Add(isReverse);
                i++;
            }
        }
    ...
    public static class Circle
    {
        ...
        public static void DrawCircle(Vector3 center, float radius, Vector3 startPos, Vector3 endPos, bool isleft, Color color, float lWidth, float yL, bool isReverse)
        {
            GameObject myline = new GameObject();
            myline.AddComponent<LineRenderer>();
            myline.tag = "Dubins";
            ...
            segmentNum = (int)(Mathf.Abs(theta) / (howDenseCurve * Mathf.PI / 180)) + 2;
        ...
        public static bool checkCircle(Vector3 center, Vector3 startPos, Vector3 endPos, bool isleft, float radius)
        {
            ...
            //for dense waypoints in curve, change "mathf.PI / 36" 
            segmentNum = (int)(Mathf.Abs(theta) / (howDenseCurve * 5 * Mathf.PI / 180)) + 1;
    ...
    public static class Dubinsmath
    {
        ...
        public static void RSLorLSR(Vector3 startPos, Vector3 goalPos, out Vector3 startTangent, out Vector3 goalTangent, float turnRadius, bool LSR)
        {
            ...
            float theta = 0;
            theta = Mathf.Acos(2 * turnRadius / D);
